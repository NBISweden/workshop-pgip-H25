...
#| eval: true
#| output: asis
tool_list <- exercise_tools$variant_filtering$advanced_filtering$tools
...
#| eval: true
#| output: asis
cat(format_modules(tool_list, bioinfo_tools=TRUE))
```

##### Virtual environment

Activate the `{{< var exercises.variant_filtering.venv >}}`
environment:

```bash
# $OPTIONS is provided via the init script
pixi shell -e {{< var exercises.variant_filtering.venv >}} $OPTIONS
```

#### {{< fa laptop >}} pixi

Copy the contents to a file `pixi.toml` in directory `{{< var
exercises.variant_filtering.dir >}}`, `cd` to directory and activate
...
#| eval: true
#| output: asis
cat(format_pixi(tool_list, "{{< var exercises.variant_filtering.dir >}}"))
```
