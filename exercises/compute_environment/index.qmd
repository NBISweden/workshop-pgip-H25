...
environments](https://pixi.sh/dev/workspace/environment/)^[[pixi](https://pixi.sh)
is a fast package management tool, similar to
[conda](https://anaconda.org/anaconda/conda).] Exercise environments
are named `e-EXERCISE-NAME` and can be activated with a custom command
line (CLI) tool called `pgip`. To activate the CLI, do the following steps:
...
two functions `pgip_elist` and `pgip_shell` and `pgip`.
`pgip_elist` lists available environments; to activate a given
environment type `pgip_shell e-EXERCISE-NAME`.
