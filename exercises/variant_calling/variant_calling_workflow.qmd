...
#| eval: true
#| output: asis
tool_list <- exercise_tools$variant_calling$variant_calling_workflow$tools
...
environment with `pixi shell`. If `{{< var exercises.variant_calling.dir
>}}/pixi.toml` already exists, simply add the three dependencies to
the `[dependencies]` section and run `pixi update`.
