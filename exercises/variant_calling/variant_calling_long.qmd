...
Retrieve the data `{{< var monkeyflower.url >}}` with `wget`:

```bash
wget {{< var monkeyflower.url >}}
...
#| echo: true
#| eval: false
module load bcftools/1.20 bwa/0.7.18 fastqc/0.12.1 \
       gatk/4.5.0.0 multiqc/1.30 picard/3.3.0 \
       QualiMap/2.2.1 samtools/1.20 \
       PDCOLD/23.12
```
...
to generate a list of *known sites* that should not be
recalibrated^[If we don't supply a list of known sites, the variants
will be treated as errors and therefore recalibrated to the reference
state.]. Therefore, we will first perform a preliminary round of
variant calling and filtering to generate a known sites callset.
...
`--create-output-variant-index` and makes `HaplotypeCaller` create a
VCF index.
...
gatk GenotypeGVCFs -OVI true -R ${REF} -V gatk-genotype-gvcf/subset.g.vcf.gz \
