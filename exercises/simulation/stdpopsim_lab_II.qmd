...
```{python}
import tsbrowse
from tsbrowse import preprocess as preprocess_
from tseda import datastore
from tseda.datastore import preprocess
import tszip

tszip.compress(ts, "data/chimp_selection.trees.tsz")
preprocess_.preprocess("data/chimp_selection.trees.tsz",
"data/chimp_selection.trees.tsb")
tsm = tsbrowse.model.TSModel("data/chimp_selection.trees.tsb")

individuals_table, sample_sets_table = preprocess(tsm)
...
```{python}
from tseda.vpages.ignn import VBar
vb = VBar(datastore=ds)
```
...
```{python}
from tseda.vpages.ignn import GNNHaplotype
gnn = GNNHaplotype(datastore=ds)
gnn.individual_id = 3
```
...
The `haplotype` index is either `0` or `1`, indicating the two
haplotypes. We plot the haplotypes:
