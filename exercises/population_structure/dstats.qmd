...
sampleinfo <- read.csv("sampleinfo.csv") %>% rename(sample=SampleAlias) %>%
  ...
  as_tibble
write.table(sampleinfo, file=paste0(PREFIX, ".pop"),
...
bcftools annotate --rename-chrs chrmap.txt $VCF -o ${PREFIX}.vcf.gz -O z -W tbi
```
...
#| echo: true
#| eval: true
plink2 --vcf ${PREFIX}.vcf.gz --allow-extra-chr \
       --set-missing-var-ids @:# \
       --make-bed \
       --out ${PREFIX} > /dev/null 2>&1
mv ${PREFIX}.fam ${PREFIX}.orig.fam
csvtk join -H -t ${PREFIX}.orig.fam ${PREFIX}.pop -f "2;1" |
    csvtk cut -t -f 7,2,3,4,5,6 |
    csvtk replace -H -t -f 6 -r 1 > ${PREFIX}.fam
plink2 --bfile ${PREFIX} --recode 12 ped \
       --out ${PREFIX} > /dev/null 2>&1
```
...
#| echo: true
#| eval: true
convertf -p par.PED.EIGENSTRAT > ${PREFIX}.convertf.log 2>&1
```
...
#| echo: true
#| eval: true
csvtk space2tab ${PREFIX}.orig.ind |
    csvtk cut -t -H -f 1,2,1 |
    csvtk replace -t -H -f 1 -p "^.*:" |
    csvtk replace -t -H -f 3 -p ":.*" > ${PREFIX}.ind
```
...
#| eval: true
fst_dir <- "fst"
extract_f2(PREFIX, fst_dir, verbose=FALSE, overwrite=TRUE)
