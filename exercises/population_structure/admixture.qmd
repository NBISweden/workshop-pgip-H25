...
(source) populations. In this exercise, we will use the method {{< var
admixture >}} [@alexander_FastModelbasedEstimation_2009] to model
ancestry.
...
- run {{< var admixture >}} to infer population structure and
...
- admixture model: we have recent ancestry from multiple source populations
- individual allele frequency P: mixture (Q) of source population P's
- first approach: estimate P from *known* populations
- STRUCTURE: estimate directly from G, Bayesian
  [@pritchard_InferencePopulationStructure_2000]

- ancestry: how much of a genome is from a set of defined groups or
  populations
- [PCA](pca.qmd) clusters individuals but says little about the
  composition of genomes

- {{< var admixture >}} is a method to estimate ancestry components
  from genotypes
- maximum likelihood
- motivation behind development: population stratification confounding
  factor in association studies

```{r, engine="tikz", fig.ext="svg" }
#| label: fig-genotype-matrix
#| echo: false
#| eval: true
#| out-width: "50%"
\begin{tikzpicture}
\tikzset{%
table/.style={matrix of math nodes,nodes in empty cells,
left delimiter={[}, right delimiter={]}},
highlight/.style={rectangle,fill=red!30,rounded corners=1mm,inner sep=0mm,outer sep=0mm,opacity=0.5},
ll/.style={node distance=4.5ex, outer sep=0mm, inner sep=0mm,font=\tiny\sffamily},
rl/.style={node distance=2.5ex, font=\tiny\sffamily}
}
 \matrix (m) [table]
  {
    a_8 & a_1 & a_6\\
  ...
  \node[highlight, fit={(m-1-1.north west) (m-3-3.south east)}]{};
\node [ll, left of=m-1-1] {1};
\node [rl, above of=m-1-1] {1};
\end{tikzpicture}
```

## Data setup

We start by defining some variables. Note that {{< var admixture >}}
...
## Running {{< var admixture >}}

Given an appropriate input file, running {{< var admixture >}} can be
done in a few steps. {{< var admixture >}} takes as input a [plink bed
...
Before running {{< var admixture >}}, we need to modify the map file
...
That is all we have to do! We next run {{< var admixture >}} with
...
Also remember that different runs of {{< var admixture >}} will
