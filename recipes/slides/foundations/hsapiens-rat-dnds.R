...
small <- "assets/data/hsapiens_rat.dnds.small.tsv"
if (!file.exists(fn)) {
  library(biomaRt)
  ensembl <- useEnsembl(
      version = 99,
...
if (!file.exists(small)) {
  hsapiens_rat <- read.table(fn, header = TRUE)
  write.table(round(hsapiens_rat[c("dnds")], digits=3),
              file = "assets/data/hsapiens_rat.dnds.small.tsv",
              sep = "\t", quote = FALSE, row.names = FALSE
              )
}
library(ggplot2)
...
ggplot(x, aes(x = dnds)) +
  geom_histogram(
      bins = 100, aes(y = after_stat(count) / sum(after_stat(count))), fill = "white",
      color = "black"
  ) +
  xlim(0, 2) +
