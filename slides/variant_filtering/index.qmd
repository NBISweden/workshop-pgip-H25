...
if [ ! -e variantsites.vcf.gz ]; then
 bcftools view data/redyellow.variantsites.vcf.gz | vcfrandomsample -r 0.1 |\
  bgzip -c > variantsites.vcf.gz
fi
if [ ! -e allsites.vcf.gz ]; then
 bcftools view data/redyellow.allsites.vcf.gz | vcfrandomsample -r 0.1 |\
...
```{r }
#| label: variant-quality-score-distribution
#| echo: true
#| eval: true
#| out-width: 80%
#| fig-width: 14
...
Missing number of sites per individual. Too many would indicate poor
sample quality.
...
Fraction missing calls per site. Could warrant separate filters when
comparing populations (e.g., total missing 0.2, but population A has
0.1 missing, population B 0.4).
...
#| echo: false
#| eval: true
REF=data/M_aurantiacus_v1.fasta
head -n 2 ${REF}
echo -e "LG4\t10\t20\nLG4\t34\t45\nLG4\t57\t59\n" | \
